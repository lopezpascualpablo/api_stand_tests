...
def test_create_user_15_letter_in_first_name_get_success_response_2():
    positive_assert("Aa")
    positive_assert("Aaaaaaaaaaaaaaa")

def negative_assert_symbol(fir_name):
    uer_body = get_user_body(fir_name)
    response = sender_stand_request.post_new_user(uer_body)
...
def egative_assert_no_firstname(user_body):
    respomse = sender_stand_request.post_new_user(user_body)
    assert respomse.status_code == 400
    assert respomse.json()["code"] == 400
    assert respomse.json()["message"] == "No se han aprobado todos los parámetros requeridos"
...
def test_create_user_no_first_name_get_error_response():
    ...
    egative_assert_no_firstname(user_body)
